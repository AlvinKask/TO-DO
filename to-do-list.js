...
addList=()=>{
   ...
   if (inputText) {
    list.innerHTML += ` <li class=" my-3 py-3 shadow list-group-item " id="list${listNum}">
                ...
                <div class="col-4">
                     <button class=" btn btn-dark" onclick="deleteList(${listNum})">Kustuta</button>
                     <button class=" btn btn-dark" onclick="editList(${listNum})">Muuda</button>
                </div>                  
                 </div>    
                </li> `;
        input.value=" ";
...
filterList=(x)=>{
       if (x) {
            if (x.length >= minimalValue) {
                ...
                alert("Palun sisesta rohkem, kui 3 tähte")
            }
       }
       else{
            return false;
...
editList=(listId)=>{
    ...
    let newText = prompt("Kas soovid muuta?",currentText.innerHTML);
...
deleteList=(listId)=>{
    let current = document.getElementById(`text${listId}`).innerHTML;
       let deleteComfirm = confirm(`Kas soovid kustutada? ${current}`);
    if (deleteComfirm) {
